...
con los paquetes y códigos necesarios para configurar streamlit.

que haga lo siguiente:

1. Crear datos sintéticos de epidemiologia.

2. Hacer EDA sobre esos datos. (cuanti, cuali, plot)

3. Permite al usuario interacción con la plataforma.

Se han creado exitosamente el archivo de dependencias requirements.txt y la aplicación interactiva de Streamlit app.py.

Archivos Generados
requirements.txt: Contiene todas las librerías necesarias para ejecutar la aplicación.
Ícono de TXT
requirements
 TXT 
Abrir

app.py: Código principal en Python configurado para Streamlit.
Ícono de PY
app
 PY 
Abrir

Funcionalidades Incluidas en app.py
Generación de Datos Sintéticos Epidemiológicos:

Creación parametrizable de un dataset con variables como: ID_Paciente, Fecha_Diagnostico, Region, Enfermedad (Dengue, Malaria, COVID-19, Influenza, Chikungunya), Edad, Genero, Estado_Vacunacion, Severidad, Hospitalizado, Ingreso_UCI y Estado_Final (Recuperado, En Tratamiento, Fallecido).

Análisis Exploratorio de Datos (EDA):

Métricas Generales (KPIs): Total de casos, tasas de hospitalización, ingreso a UCI y letalidad.

Análisis Cuantitativo: Estadísticas descriptivas de edad (media, mediana, desviación, cuartiles), distribución por rangos etarios y promedios agrupados por enfermedad.

Análisis Cualitativo: Tablas cruzadas y distribuciones de frecuencia de variables categóricas (ej. Enfermedad vs. Severidad, Vacunación vs. Estado Final).

Visualización e Interacción:

Filtros Interactivos: Controles en la barra lateral para ajustar el tamaño de la muestra, semilla aleatoria (seed), rango de fechas, regiones, enfermedades y niveles de severidad.

Gráficos Dinámicos en Plotly:

Curva epidemiológica semanal.

Gráfico de cajas (Boxplot) de edad según severidad.

Diagrama de barras por estado final y región.

Gráfico circular (Donut) del estado de vacunación.

Mapa de calor (Heatmap) de interacción entre enfermedad y severidad.

Descarga de Datos: Botón interactivo para exportar el dataset filtrado en formato CSV.
